...
export default function DiskusiAndProjectPage() {
  ...
  return (
    <>
      <NavLayanan />
      <main className="flex-2 px-4 py-2 lg:px-20 lg:py-7 lg:ml-[20%]">
        {/* Navigasi Form */}
        <div className="flex gap-4 mt-[100px] text-white text-[3.5vw] md:text-[3vw] lg:text-[1.3vw]">
          <span
          onClick={() => setActiveForm("diskusi")}
          className={`cursor-pointer hover:underline  
            ${activeForm === 'diskusi' ? 'font-bold text-white' : 'font-reguler text-[#cccccc]'}
            `}
          >
            Pengajuan Diskusi
          </span>
          ...
          onClick={() => setActiveForm("project")}
                    className={`cursor-pointer hover:underline  
                      ${activeForm === 'project' ? 'font-bold text-[#ffffff]' : 'font-reguler text-[#cccccc]'}
                      `}>
            Pengajuan Project
          </span>
        </div>
